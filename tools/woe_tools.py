...
def numwoe_autobinning(df_master, var_name, target, max_bins=6, min_prop_in_bin=0.05, missing=True, max_bin_init=200,
    ...
    if method == 'chisq':
        ...
        max_depth = int(np.log(max_bins, 2))
        if missing:
...
def numwoe_aptbinning(df_master, var_name, target, bins, missing=True, to_plot=True, to_show=True, save_path=None):
    """
    ...
    df_ref_table: DataFrame
        woe reference table
    """
    ds = init_binning(df_master, var_name=var_name, target=target, cut_points=bins, missing=missing)
    df_ref_table = __gene_reference(ds, var_name=var_name, var_type='numerical')
    if to_plot:
        plot_reference(df_ref_table, to_show=to_show, save_path=save_path)
...
def catwoe_aptbinning(df_master, var_name, target, bins, to_plot=True, to_show=True, save_path=None):
    """
    根据指定的分组对categorical变量进行分bin，并计算WOE
    Parameters
    ----------
    df_master: DataFrame
    var_name: str
...
def numwoe_apply(df_master, ref_table, var_name, overwrite=False, prefix='nwoe_'):
    ...
    res = np.dot(x, w) + missing_woe * df_master[var_name].isnull().values
    if overwrite:
        df_master[var_name] = res
    else:
        df_master[prefix + var_name] = res
